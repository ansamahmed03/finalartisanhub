            ...
            <div class="card card-primary">
                ...
                <form>
                    <div class="card-body">
                            ...
                            <select class="form-control" id="customer_id">
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}"
                                        @if($customer->id == $booking->customer_id) selected @endif>
                                        {{ $customer->email }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-control" id="team_id">
                                @foreach($teams as $team)
                                    <option value="{{ $team->id }}"
                                        @if($team->id == $booking->team_id) selected @endif>
                                        {{ $team->team_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="status">
                                @foreach(['pending','confirmed','completed','cancelled'] as $status)
                                    <option value="{{ $status }}"
                                        @if($booking->status == $status) selected @endif>
                                        {{ ucfirst($status) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            <label>Notes</label>
                            <textarea class="form-control" id="notes" rows="3">{{ $booking->notes }}</textarea>
                        </div>

                    </div>
                    <div class="card-footer">
                        <button type="button" onclick="performUpdate({{ $booking->id }})" class="btn btn-primary">Update</button>
                        <a href="{{ route('booking.index') }}" class="btn btn-secondary">Go To Index</a>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    function performUpdate(id) {
        ...
        storeRoute('/cms/Admin/booking_update/' + id, formData);
    }
</script>
@endsection
